...
### گزارشِ عیب‌یابی (لاگِ کاربران)
- `POST /api/diag` `{id?, version?, report}` → کلاینت گزارشِ متنیِ لاگش را می‌فرستد (در حافظه، آخرین ۳۰۰ مورد).
- `GET  /api/diag?key=<DIAG_KEY>&after=<ms>&id=<id>` → فقط توسعه‌دهنده (با `DIAG_KEY` در env) گزارش‌ها را می‌خواند. بدونِ تنظیمِ `DIAG_KEY` خواندن **غیرفعال** است (۴۰۳).

برای دیدنِ خطاهای کاربران: `DIAG_KEY` را در env تنظیم کنید، سپس `curl "https://.../api/diag?key=KEY"`.
