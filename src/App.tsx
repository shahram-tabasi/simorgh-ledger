...
const CHANGELOG: string[] = [
  'حسابداری: ترازِ گروه‌بندی‌شده‌ی چندسطحی (گروه/کل/معین) با جمعِ هر سرفصل',
  'بستنِ سالِ مالی: سندِ اختتامیه‌ی خودکار و انتقالِ سود/زیان به «سود و زیانِ انباشته»',
  'اظهارنامه‌ی مالیات بر ارزش افزوده: مالیاتِ فروش − مالیاتِ خرید = قابلِ پرداخت/طلب',
];
      ...
      {showLeftDrawer && (
        <div className="drawer-overlay" onClick={() => setShowLeftDrawer(false)}>
          <aside className="drawer drawer-left" onClick={e => e.stopPropagation()}>
            ...
            <div className="drawer-foot">نسخه ۱۴۰۵ · ۱.۰.۶۰</div>
          </aside>
        </div>
      )}
