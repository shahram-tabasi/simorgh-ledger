...
const CHANGELOG: string[] = [
  'رفعِ خطای نصب و امکانِ آپدیتِ روان: نرم‌افزار حالا با امضای ثابتِ رسمی ساخته می‌شود',
  '(برای این‌بار: اگر نسخه‌ی قبلی نصب است، یک‌بار آن را پاک کنید و این نسخه را نصب کنید؛ از این به بعد آپدیت‌ها بدون مشکل انجام می‌شود)',
];
      ...
      {showLeftDrawer && (
        <div className="drawer-overlay" onClick={() => setShowLeftDrawer(false)}>
          <aside className="drawer drawer-left" onClick={e => e.stopPropagation()}>
            ...
            <div className="drawer-foot">نسخه ۱۴۰۵ · ۱.۰.۲۴</div>
          </aside>
        </div>
      )}
